...
────                        ─────────────                  ─────────────────────
Pin 4 (+12V) ─────────────► [buck converter, set to 5V] ─► VCC (5V) pin
Pin 1 (GND)  ───────────────────────────────────────────── GND pin
...
| Ref | Value | Purpose |
| --- | --- | --- |
| U1 | XL7015 buck module (or similar, ≥0.8 A) | 12 V → 5 V board power. Trim to 5.0 V **before** connecting the ESP32. |
...
converter. Budget ≥500 mA at 5 V (ESP32-S3 with WiFi active plus the
display backlight). Set-up procedure for the buck module:

1. Power it from a bench supply at 12 V with no load.
2. Trim the output to 5.0 V.
3. Connect the ESP32, boot it with WiFi up, and re-measure — re-trim if the
   output sags below ~4.85 V.
...
- [ ] Buck output 5.0 V unloaded, ≥4.85 V with the controller booted and WiFi up
