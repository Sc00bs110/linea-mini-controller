...
// disables WiFi sleep, and runs the HTTPUpdate. Plain http:// uses a WiFiClient;
// https:// uses WiFiClientSecure + setInsecure() and follows redirects (only
// compiled when FEATURE_GH_OTA is defined — brings in the TLS stack). On success
// the device reboots into the new image; on any failure it logs and reboots.
void ota_http_tick();
