...
#include <Update.h>   // Update.abort() — clear a wedged Update state on OTA error
#include <lvgl.h>
...
// MQTT-triggered HTTP-pull OTA (queued in the MQTT callback, run from loop()).
#include "ota_http.h"
...
static void ota_task(void*) {
    ...
    ArduinoOTA.onStart([]() {
        ...
        // Fully release the radio: pausing the BLE scan is not enough on the
        // S3's single shared 2.4GHz radio — NimBLE must be deinitialized to free
        // it (proven 2026-07-08: transfers died mid-stream while BLE was live).
        scale_radio_release();
        // Legal only after NimBLEDevice::deinit() — throws while BLE is init'd.
        WiFi.setSleep(false);
        ui_ota_start();
    });
    ...
    ArduinoOTA.onError([](ota_error_t e) {
        ...
        // #8393) so every later attempt instant-aborts. Abort to clear it, then
        // reboot for a clean radio/BLE state on the next attempt.
        Update.abort();
        ESP.restart();
    });
...
void loop() {
    ...
    wifi_tick();
    mqtt_tick();
    ota_http_tick();   // service any MQTT-queued HTTP-pull OTA request
    wlog_tick();
